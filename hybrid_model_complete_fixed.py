...
class HybridCarbonForecaster(BaseEstimator, RegressorMixin):
    ...
    def _transform_features(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.use_tcn and self.tcn_extractor is not None:
            ...
            self.tcn_extractor.model.eval()
            self.tcn_extractor.projection.eval()
        ...
        if self.use_attention and self.attention_weighter is not None:
            # Ensure eval mode for deterministic behavior
            self.attention_weighter.model.eval()
    ...
    def save(self, filepath: str):
        """Save model to file with complete state preservation.
        ...
        if self.use_tcn and self.tcn_extractor is not None:
            ...
            model_dict['tcn_state'] = {
                ...
                'optimizer_state': self.tcn_extractor.optimizer.state_dict(),
        ...
        # Save Attention state with optimizer
        if self.use_attention and self.attention_weighter is not None:
            # Set to eval mode before saving
            self.attention_weighter.model.eval()
            ...
            model_dict['attention_state'] = {
                ...
                'optimizer_state': self.attention_weighter.optimizer.state_dict(),
    ...
    def load(cls, filepath: str) -> 'HybridCarbonForecaster':
        """Load model from file with complete state restoration.
        
        Args:
            filepath: Path to load model from
        
        Returns:
            Loaded model with identical behavior
        """
        ...
        if 'tcn_state' in model_dict and model.use_tcn:
            ...
            if 'optimizer_state' in tcn_state:
                model.tcn_extractor.optimizer.load_state_dict(tcn_state['optimizer_state'])
            
            # Restore metadata
            model.tcn_extractor.is_fitted = tcn_state['is_fitted']
            
            # CRITICAL: Set to evaluation mode and disable dropout
            model.tcn_extractor.model.eval()
            model.tcn_extractor.projection.eval()
            
            # Disable gradients for inference
            for param in model.tcn_extractor.model.parameters():
                param.requires_grad = False
            for param in model.tcn_extractor.projection.parameters():
                param.requires_grad = False
        ...
        if 'attention_state' in model_dict and model.use_attention:
            ...
            # Load optimizer state
            if 'optimizer_state' in attn_state:
                model.attention_weighter.optimizer.load_state_dict(attn_state['optimizer_state'])
            
            # Restore metadata
            model.attention_weighter.trained = attn_state['trained']
            
            # CRITICAL: Set to evaluation mode and disable dropout
            model.attention_weighter.model.eval()
            
            # Disable gradients for inference
            for param in model.attention_weighter.model.parameters():
                param.requires_grad = False
